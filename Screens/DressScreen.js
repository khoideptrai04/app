  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch the category_id for 'Dress'
        ...
        if (categoryError || !categoryData) {
          console.error('Error fetching Dress category:', categoryError);
          throw new Error('Dress category not found');
        }
        ...
        if (productError) {
          console.error('Error fetching Dresses:', productError);
          throw new Error(`Failed to fetch Dresses: ${productError.message}`);
        }
        ...
        const formattedProducts = productData.map((item) => ({
          ...
          name: item.name,
          price: item.price,
          image: item.image_url || 'dress.png',
        ...
        Alert.alert('Error', `Failed to load Dresses: ${error.message}`);
      } finally {
        setLoading(false);
      }
    };
    ...
    <SafeAreaView style={styles.container}>
      ...
      {loading ? (
        ...
        <View style={styles.emptyContainer}>
          <Text style={styles.emptyText}>No Dresses available.</Text>
        </View>
      ) : (
        <FlatList
          data={products}
